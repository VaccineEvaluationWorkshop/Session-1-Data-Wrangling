...
ts.plot.fun <- function(time.var=date, yvar,multiple_group=F, group.var, vax.intro.date ){
...
ALTERNATIVELY: we could do it using the ts_format function in the package ExcessILI
...
e2.m <- reshape2::melt(e2[,c('Age.group','date',log_var_names)], id.vars=c('Age.group','date'))
...
```{r}
p4 <- ggplot(e2.m, aes(x=date, y=value, group=variable)) +
      geom_line(col='blue')  +
      ylab("Death by cause") +  #add y axis label
      xlab("Date") +   #add x axis label
      theme_classic() +  #choose a 'theme'
      geom_vline(xintercept=as.Date('2006-01-01'), col='gray', lty=2) + #add reference line for vaccine introduction
      facet_wrap(~Age.group, scales='free_y') + #different plot for each age group, each with unique scale (scale='free')
      theme(panel.spacing= unit(2,'lines') , axis.text.x=element_text(angle=90))
