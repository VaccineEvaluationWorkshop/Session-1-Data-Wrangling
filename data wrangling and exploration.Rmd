...
ts.plot.fun <- function(time.var=date, yvar,multiple_group=F, group.var, vax.intro.date=as.Date('2006-01-01') ){
...
```{r, eval=F}
d2<-aggregate(d1[ ,c('j12_j18','A15_A19_prim','A00_B99_prim','p_prim','q_prim','r_prim','w_prim','g_prim')], by=list('agec'=d1$Age.group, 'date'=d1$date), FUN=sum)
```
...
```{r}
d2.spl<-split(d2, d2$Age.group) #split the dataset up by age group
```
...
d2.spl<- lapply(d2.spl, function(x) {
  y<-apply(x[,6:ncol(x)],2, function(x2) scale(log(x2+0.5)) )
  z<-cbind.data.frame(x[,c('Age.group','date','state','j12_j18')],y)
  return(z)
}
)
```

Plot the controls. From these plots, it looks like the counts fall off sharply in the last few months, which likely indicates a delay in reporting to the system. We might want to exclude more recent time periods to avoid bias related to reporting delays.
```{r, fig.width=6, fig.height=6}
par(mfrow=c(2,2))
lapply(d2.spl, function(x){
matplot(x[,5:ncol(x)], type='l', bty='l')
